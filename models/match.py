...
class Match:
    def __init__(self, player_a: Player, player_b: Player, score_a="", score_b=""):
        ...
        self.players_match = f"Match : *id: {self.player_a.index} - {self.player_a.fullname}* VERSUS *id: {self.player_b.index} - {self.player_b.fullname}*"
    ...
    def enter_score(self):
        ...
        # self.score_b = float(input(f"Enter score of {self.player_b.fullname} [1 for winner/0 for loser/0.5 for draw] :  "))
    ...
    def display_players_score(self):
        self.players_scores = ([self.player_a.index, self.score_a], [self.player_b.index, self.score_b])
